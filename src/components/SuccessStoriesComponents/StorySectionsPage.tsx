...
export function Section({
  ...
  title?: string;
  description?: string | PortableTextBlock[]; // ✅ string or PortableText blocks
  items?: SectionItem[];
}) {
  ...
  return (
    <section
      id="problem"
      className="scroll-mt-36 mt-10 sm:mt-14 sm:max-w-8xl sm:mx-auto sm:px-4 sm:mb-10"
    >
      {title && (
        <h2 className="text-2xl sm:text-2xl text-center font-semibold text-gray-800 mb-6">
          {title}
        </h2>
      )}
      ...
      <div className="px-2 sm:px-16">
        {description && (
          <div className="text-gray-400 text-md leading-tight sm:leading-snug text-left sm:text-center max-w-3xl mx-auto mb-6">
...
export function SectionNew({
  ...
  return (
    <section id="solution" className="scroll-mt-36 mt-10 max-w-8xl mx-auto sm:px-4">
...
export function ListSection({
  ...
  title?: string;
  images: GridImage[];
  items: (ListItem & { description?: string | PortableTextBlock[] })[]; // ✅ support both
}) {
  if (!title && !images.length && !items.length) return null;

  return (
    <section id="challenges" className="scroll-mt-36 sm:m-20 mt-10 ">
      {title && (
        <h2 className="text-2xl text-gray-800 font-semibold text-center sm:mb-8 mb-4">
          {title}
        </h2>
      )}
      ...
      <ul className="grid grid-cols-2 gap-x-3 gap-y-4 mt-4 sm:mt-8 sm:mx-16 sm:grid-cols-2 lg:grid-cols-3 sm:gap-7 sm:gap-y-5">
        {items.map((c, i) => (
          <li
            key={i}
            className="p-3 sm:p-5 border border-gray-200 rounded-xl shadow-sm transition-shadow"
          >
            <h2 className="font-semibold text-gray-900 sm:mb-4 sm:mt-2 mb-2 leading-[18px] ">
              {c.title}
            </h2>
            <div className="text-gray-400 text-sm tracking-[0.1px] leading-[18px] sm:leading-[16px] sm:tracking-[0.4px]">
...
export function ListSectionNew({
  ...
  title?: string;
  images: GridImage[];
  items: (ListItem & { description?: string | PortableTextBlock[] })[]; // ✅ allow both string & block content
}) {
  if (!title && !images.length && !items.length) return null;
        ...
        <ul className="space-y-6 flex-[2] self-center">
          {items.map((c, i) => (
            <li key={i}>
              ...
              <div className="text-sm text-gray-400 leading-[18px] tracking-[0.1px] sm:leading-[16px] sm:tracking-[0.4px]">
                {Array.isArray(c.description) ? (
                  <PortableText value={c.description} />
                ) : (
                  <p>{c.description}</p>
...
// function renderContent(content: string | PortableTextBlock[] | undefined) {
//   if (!content) return null;

//   if (typeof content === "string") {
//     return (
//       <p className="text-gray-400 font-[6px] tracking-[.1px] leading-[18px]">
//         {content}
//       </p>
//     );
//   }
...
function renderContent(
  content: string | PortableTextBlock[] | undefined
): ReactNode {
  if (!content) return null;

  if (typeof content === "string") {
    return (
      <p className="text-gray-400 sm:leading-[16px] leading-[18px] tracking-tight">
        {content}
      </p>
    );
  }
  ...
  if (Array.isArray(content)) {
    return (
      <div className="text-gray-400 sm:leading-[16px] leading-[18px] tracking-tight">
...
export function ListNew({ items }: { items: ListItem[] }) {
  return (
    <ul className="grid grid-cols-2 md:grid-cols-2 sm:gap-y-3 sm:gap-x-24 text-left sm:mx-44 sm:mt-8">
      {items?.map((item, i) => (
        <li key={i} className="p-4 mt-2 rounded-x">
